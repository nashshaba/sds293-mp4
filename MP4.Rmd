...
CA<-training %>%
  mutate(type_hat = type_hat_class$value,
         true = ifelse(type_hat==type, 1, 0))%>%
  summarise(mean(true==1))
```
...
colnames(predicted_points_optimal)[1]<-"id"

write_csv(predicted_points_optimal, path = "data/submission.csv")
```
